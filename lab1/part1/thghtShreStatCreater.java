...
public class thghtShreStatCreater {
      ...
      try {
         ...
         while (scanner.hasNextLine()) {
           ...
           /* Add user to set to keep track of uniqueness */
            ...
            processStatus(status, numChars, wordCnt);
            processUser(user);
            processRecipient(recipient, wordCnt, numChars);  
         }
      }
      catch (Exception e) {
   ...
   private void processStatus(String status, int charLength, int wordLength) {
   ...
   private void processRecipient(String recipient, int numWords, int numChars) {
       ...
       */
       /* possible recipient types: {subscribers, self, a userId, all} */
      if (recipient.compareTo("subscribers") == 0) {
         subWordCount.add(numWords);
         subCharCount.add(numChars);
      }
      else if (recipient.compareTo("self") == 0) {
         selfWordCount.add(numWords);
         selfCharCount.add(numChars);
      }
      else if (recipient.compareTo("all") == 0) {
